...
class GameViewController(
    ...
    private val bgImage1: ImageView, private val bgImage2: ImageView
) {
    ...
    // Only enable first row opn board
    def enableFirstRow() {
    ...
    def showAnswer(){
        for (i <- 0 to 3){
            ...
            answerList(i).setFill(Color.web(game.player.board.answer(i).hexValue))
        }
    }
    ...
    def handleCheck(action: ActionEvent){
        ...
        //If correct answer
        if (game.player.guess == game.player.board.answer){
            game.player.won = true
            showAnswer()
            for (i <- 0 to 3){
                game.player.feedback.update(i, game.player.board.feedbackPegs(0))
            }
            showFeedback()
            MainApp.showEndScreen(game.isWon())
        }
        // Wrong answer
        else {
            var answerClone = game.player.board.answer.clone()
            // Check if correct color, correct position
            for (i <- 0 to 3){
                if (game.player.guess(i) == game.player.board.answer(i)){
                    game.player.feedback.append(game.player.board.feedbackPegs(0)) 
                    game.player.feedback.remove(0)
                    answerClone -= game.player.guess(i)
                }
            }

            // Check if correct color, wrong position
            for (i <- 0 to 3){
                if (answerClone.contains(game.player.guess(i))){
                    game.player.feedback.append(game.player.board.feedbackPegs(1)) 
                    game.player.feedback.remove(0)
                    answerClone -= game.player.guess(i)
                }
            }
            ...
            for (i <- answerClone){
                game.player.feedback.append(game.player.board.feedbackPegs(2)) 
                game.player.feedback.remove(0)
            }
            ...
            // If player has lost
            if (game.player.guessCount == game.player.board.rows){
    ...
    def clearGuess(){ // Set guess list to all White Pegs
        for (i <- 0 to 3){
            game.player.guess.update(i, game.player.board.feedbackPegs(2))
        }
    }

    def clearGuessView(){ // Set all guess opacity to 0 and color to White
        for (i <- 0 to 3){
            ...
            guess1(i).setFill(Color.web(game.player.board.feedbackPegs(2).hexValue))
            guess2(i).setFill(Color.web(game.player.board.feedbackPegs(2).hexValue))
            guess3(i).setFill(Color.web(game.player.board.feedbackPegs(2).hexValue))
            guess4(i).setFill(Color.web(game.player.board.feedbackPegs(2).hexValue))
            guess5(i).setFill(Color.web(game.player.board.feedbackPegs(2).hexValue))
            guess6(i).setFill(Color.web(game.player.board.feedbackPegs(2).hexValue))
            guess7(i).setFill(Color.web(game.player.board.feedbackPegs(2).hexValue))
            guess8(i).setFill(Color.web(game.player.board.feedbackPegs(2).hexValue))
            guess9(i).setFill(Color.web(game.player.board.feedbackPegs(2).hexValue))
            guess10(i).setFill(Color.web(game.player.board.feedbackPegs(2).hexValue))
        }
    }

    def clearFeedback(){ // Set feedbacklist to all White Pegs
        for (i <- 0 to 3){
            game.player.feedback.update(i, game.player.board.feedbackPegs(2))
        }
    }

    def clearFeedbackView(){ // Set all feedback opacity to 0 and color to White
        for (i <- 0 to 3){
            ...
            feedback1(i).setFill(Color.web(game.player.board.feedbackPegs(2).hexValue))
            feedback2(i).setFill(Color.web(game.player.board.feedbackPegs(2).hexValue))
            feedback3(i).setFill(Color.web(game.player.board.feedbackPegs(2).hexValue))
            feedback4(i).setFill(Color.web(game.player.board.feedbackPegs(2).hexValue))
            feedback5(i).setFill(Color.web(game.player.board.feedbackPegs(2).hexValue))
            feedback6(i).setFill(Color.web(game.player.board.feedbackPegs(2).hexValue))
            feedback7(i).setFill(Color.web(game.player.board.feedbackPegs(2).hexValue))
            feedback8(i).setFill(Color.web(game.player.board.feedbackPegs(2).hexValue))
            feedback9(i).setFill(Color.web(game.player.board.feedbackPegs(2).hexValue))
            feedback10(i).setFill(Color.web(game.player.board.feedbackPegs(2).hexValue))
        }
    }

    // When user clicks on a color 
    def copyColor1(){        
        game.player.pegSelected = game.player.board.pegList(0)
    }
    def copyColor2(){        
        game.player.pegSelected = game.player.board.pegList(1)
    }
    def copyColor3(){        
        game.player.pegSelected = game.player.board.pegList(2)
    }
    def copyColor4(){        
        game.player.pegSelected = game.player.board.pegList(3)
    }
    def copyColor5(){        
        game.player.pegSelected = game.player.board.pegList(4)
    }
    def copyColor6(){        
        game.player.pegSelected = game.player.board.pegList(5)
    }
    def copyColor7(){        
        game.player.pegSelected = game.player.board.pegList(6)
    }

    // when user places the color on a peg 
    def pasteColor1(){
        game.player.guess.update(0, game.player.pegSelected)
        if (game.player.guessCount == 0){
            guess1_1.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 1){
            guess2_1.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 2){
            guess3_1.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 3){
            guess4_1.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 4){
            guess5_1.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 5){
            guess6_1.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 6){
            guess7_1.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 7){
            guess8_1.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 8){
            guess9_1.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 9){
            guess10_1.setFill(Color.web(game.player.pegSelected.hexValue))
        } 
    }
    def pasteColor2(){
        game.player.guess.update(1, game.player.pegSelected)
        if (game.player.guessCount == 0){
            guess1_2.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 1){
            guess2_2.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 2){
            guess3_2.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 3){
            guess4_2.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 4){
            guess5_2.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 5){
            guess6_2.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 6){
            guess7_2.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 7){
            guess8_2.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 8){
            guess9_2.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 9){
            guess10_2.setFill(Color.web(game.player.pegSelected.hexValue))
        }
    }
    def pasteColor3(){
        game.player.guess.update(2, game.player.pegSelected)
        if (game.player.guessCount == 0){
            guess1_3.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 1){
            guess2_3.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 2){
            guess3_3.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 3){
            guess4_3.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 4){
            guess5_3.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 5){
            guess6_3.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 6){
            guess7_3.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 7){
            guess8_3.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 8){
            guess9_3.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 9){
            guess10_3.setFill(Color.web(game.player.pegSelected.hexValue))
        }
    }
    def pasteColor4(){
        game.player.guess.update(3, game.player.pegSelected)
        if (game.player.guessCount == 0){
            guess1_4.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 1){
            guess2_4.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 2){
            guess3_4.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 3){
            guess4_4.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 4){
            guess5_4.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 5){
            guess6_4.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 6){
            guess7_4.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 7){
            guess8_4.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 8){
            guess9_4.setFill(Color.web(game.player.pegSelected.hexValue))
        } else if (game.player.guessCount == 9){
            guess10_4.setFill(Color.web(game.player.pegSelected.hexValue))
        }
    }

    // Show feedback on board
    def showFeedback(){
